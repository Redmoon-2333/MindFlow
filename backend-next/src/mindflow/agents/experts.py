...
{
  "approved": true|false,
  "issues": ["问题1描述（每条 ≤50 字；无问题时为空数组）"],
  "critique_detail": "简洁的审查说明（中文，≤300 字）"
}

## 输出长度硬约束
- 整个 JSON 必须精简：critique_detail 不超过 300 字，issues 每条不超过 50 字。
- 严禁长篇罗列或逐证据复读——你是快速质检员，不是写审查报告。
- 输出过长导致 JSON 被截断会直接作废，因此务必控制篇幅。
