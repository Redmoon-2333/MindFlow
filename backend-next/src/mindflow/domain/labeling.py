...
class LabelingSignal(Protocol):
    """One weak supervision signal that votes focus(1) or distraction(0).

    Each signal returns (vote, confidence) where confidence in [0, 1].
    """

    def __call__(self, row: Mapping[str, Any]) -> tuple[int, float]: ...
...
class ApplicationDiversitySignal:
...
class EntertainmentDominanceSignal:
...
class ConsensusLabeler:
    ...
    def __init__(self, signals: Sequence[LabelingSignal] | None = None) -> None:
        if signals is not None:
            self.signals: list[LabelingSignal] = list(signals)
        else:
            self.signals = [
                ProductivitySignal(),
    ...
    def label_dataframe(self, rows: Sequence[Mapping[str, Any]]) -> tuple[list[int], list[float]]:
        """Label all rows in a feature list.

        Returns:
            labels: list of int (1=focus, 0=distraction)
    ...
    def label_with_confidence_split(
        self, rows: Sequence[Mapping[str, Any]], min_confidence: float = 0.5
        ...
        Returns:
            (high_labels, high_confs, low_labels, low_confs)
        """
        all_labels, all_confs = self.label_dataframe(rows)
        ...
        for lbl, conf in zip(all_labels, all_confs, strict=True):
            if conf >= min_confidence:
                high_labels.append(lbl)
                high_confs.append(conf)
            else:
                low_labels.append(lbl)
                low_confs.append(conf)

        return high_labels, high_confs, low_labels, low_confs

    def signal_breakdown(self, row: Mapping[str, Any]) -> dict[str, dict[str, float]]:
        """Return per-signal votes for inspection."""
        return {
            type(s).__name__: {"vote": float(v), "confidence": c}
