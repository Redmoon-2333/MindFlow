...
Covers:
  - v2 delegation path: AnalysisWorkflowPort (AnalysisGraph) → PanelVerdict
  - Fallback: no workflow port → single-expert LLM service
...
class TestPanelServiceFallback:
    """No workflow port injected → single-expert LLM fallback."""
    ...
    async def test_fallback_uses_single_expert_llm(
        self, panel_service: PanelService
    ) -> None:
        """No port → llm_service.analyze called, verdict built from outcome."""
        verdict = await panel_service.run_daily_panel(
            user_id=1, target_date=date(2026, 7, 18),
        )
        assert verdict.source == "single_expert"
        assert verdict.types == (ProcrastinationType.IMPULSIVITY,)
        assert verdict.call_count == 0
        # Verify LLM service was called with daily_panel kind
        panel_service._llm_service.analyze.assert_awaited_once()
        kwargs = panel_service._llm_service.analyze.await_args.kwargs
        assert kwargs["analysis_kind"] == "daily_panel"

    async def test_fallback_rationale(self, panel_service: PanelService) -> None:
        """Fallback verdict has the fallback LLM's rationale."""
        verdict = await panel_service.run_daily_panel(
            user_id=1, target_date=date(2026, 7, 18),
        )
        assert verdict.rationale == "单专家分析结果（降级模式）"

    async def test_fallback_no_transcript(self, panel_service: PanelService) -> None:
        """Fallback verdict has no transcript entries."""
        verdict = await panel_service.run_daily_panel(
            user_id=1, target_date=date(2026, 7, 18),
        )
...
class TestPanelServiceWorkflowPort:
    ...
    async def test_workflow_port_delegation(self) -> None:
        ...
        assert req.analysis_kind == "daily_panel"
        ...
        # Delegation means the LLM fallback was NOT called
        service._llm_service.analyze.assert_not_called()

    async def test_workflow_port_none_uses_llm_fallback(self) -> None:
        """None workflow_port → single-expert LLM fallback path."""
        service = PanelService.__new__(PanelService)
        service._builder = AsyncMock()
        service._llm_service = AsyncMock()
        service._llm_service.analyze.return_value = type(
            "Outcome",
            (),
            {
                "assessment": {
        ...
        verdict = await service.run_daily_panel(user_id=1, target_date=date(2026, 7, 26))

        # Fallback path used → llm_service.analyze called
        service._llm_service.analyze.assert_awaited_once()
        assert verdict.source == "single_expert"
