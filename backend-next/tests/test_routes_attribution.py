...
def _make_app(llm_service_mock=None, *, timezone: str = "local") -> FastAPI:
    ...
    app.state.settings = SimpleNamespace(timezone=timezone)
...
class TestAttributionRoute:
    ...
    def test_success_defaults_to_business_today(self, monkeypatch) -> None:
        """Without body, defaults to today in the configured business timezone."""
        mock_service = MagicMock()
        mock_service.analyze = AsyncMock(return_value=_success_outcome())
        ...
        app = _make_app(mock_service, timezone="Asia/Shanghai")
        client = TestClient(app)
        ...
        assert mock_service.analyze.await_args.kwargs["target_date"] == date(2026, 7, 26)
