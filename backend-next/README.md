...
- **输入与浏览器数据最小化**：输入侧仅存储 **30 秒聚合计数**（`keypress_count / mouse_click_count / scroll_delta / mouse_distance_px / input_active_s`，不含按键字串与坐标）；浏览器侧仅存域名与时长与是否外放，不存完整 URL。输入桶默认保留 7 天（可选 1-30 天）、活动与浏览器片段默认 30 天（可选 7-90 天）。
- **LLM 隐私保护**：LLM 分析仅发送聚合后的行为摘要（无窗口标题、文件路径等敏感信息）。摘要中包含的是匿名化的指标数据（切换频率、专注时长比例等）。
- **可选网络功能**：LLM 增强分析需要网络连接（DeepSeek API 或 Ollama），用户可以随时关闭；`设置` 顶部隐私卡片可一键关闭输入统计与浏览器域名统计、生成 8 字符无歧义配对码（5 分钟有效、限试 5 次）。
- **数据控制权**：用户可通过导出功能随时获取完整数据副本，并可在 `设置 → 隐私行为采集 / 浏览器追踪` 快速关闭对应来源、`设置 → 数据导出` 按范围导出并通过 `telemetry/data?scope` 按 `interaction / browser / feedback / all` 精准清理。
